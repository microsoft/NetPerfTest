...
.DESCRIPTION
    ...
    to invoke the commands that were generated above, we pass the same parameters to ProcessCommands function
    Note that we expect the directory to be pointing to the folder that was generated by perftest.ps1 under the outpurDir path supplied by the user
    This function processes all commands in 1 PS Session due to server issues
    Ex: ProcessCommands -DestIp "$DestIp" -SrcIp "$SrcIp" -CommandsDir "C:\temp\msdbg.Machine1.perftest" -DestIpUserName "domain\username" -SrcIpUserName "domain\username"
    You may chose to run SetupTearDown.ps1 -Cleanup if you wish to clean up any config changes from the Setup step
#>
Function ProcessCommandsSSH{
    param(
    [Parameter(Mandatory=$True)]  [string]$DestIp,
    [Parameter(Mandatory=$True)] [string]$SrcIp,
    [Parameter(Mandatory=$True)]  [string]$CommandsDir,
    [Parameter(Mandatory=$True, Position=0, HelpMessage="Dest Machine Username?")]
    [string] $DestIpUserName,
    [Parameter(Mandatory=$True, Position=0, HelpMessage="Dest Machine KeyPath?")]
    [string] $DestKeyPath,
    [Parameter(Mandatory=$True, Position=0, HelpMessage="Src Machine Username?")]
    [string] $SrcIpUserName,
    [Parameter(Mandatory=$True, Position=0, HelpMessage="Src Machine KeyPath?")]
    [string]$SrcKeyPath,
    [Parameter(Mandatory=$False)] [string]$Bcleanup=$True,
    [Parameter(Mandatory=$False)]$ZipResults=$True,
    [Parameter(Mandatory=$False)]$TimeoutValueInSeconds=90,
    ...
    try {
        ...
        $recvPSSession = New-PSSession -HostName $DestIp -Username $DestIpUserName -KeyFilePath $DestKeyPath

        if($recvPSsession -eq $null) {
            LogWrite "Error connecting to Host: $($DestIp)"
            return
        }

        # Establish the Remote PS session with Sender
        $sendPSSession = New-PSSession -HostName $SrcIp -Username $SrcIpUserName -KeyFilePath $SrcKeyPath

        if($sendPSsession -eq $null) {
            LogWrite "Error connecting to Host: $($SrcIp)"
            return
        }
        ...
        if (Test-Path -Path "$commandsDir\ctstraffic") {
            LogWrite "Processing ctsTraffic commands" $true 
            ProcessToolCommandsWithPSSession -Toolname "ctsTraffic" -RecvPSSession $recvPSSession -SendPSSession $sendPSSession -CommandsDir $workingDir -Bcleanup $Bcleanup -BZip $ZipResults -TimeoutValueBetweenCommandPairs $TimeoutValueInSeconds -PollTimeInSeconds $PollTimeInSeconds 
        }
    
        if (Test-Path -Path "$commandsDir\cps") {
            LogWrite "Processing cps commands" $true
            ProcessToolCommandsWithPSSession -Toolname "cps" -RecvPSSession $recvPSSession -SendPSSession $sendPSSession -CommandsDir $workingDir -Bcleanup $Bcleanup -BZip $ZipResults -TimeoutValueBetweenCommandPairs $TimeoutValueInSeconds -PollTimeInSeconds $PollTimeInSeconds 
        }

        if (Test-Path -Path "$commandsDir\ntttcp") {
            LogWrite "Processing ntttcp commands" $true
            ProcessToolCommandsWithPSSession -Toolname "ntttcp" -RecvPSSession $recvPSSession -SendPSSession $sendPSSession -CommandsDir $workingDir -Bcleanup $Bcleanup -BZip $ZipResults -TimeoutValueBetweenCommandPairs $TimeoutValueInSeconds -PollTimeInSeconds $PollTimeInSeconds 
        }

        if (Test-Path -Path "$commandsDir\latte") {
            LogWrite "Processing latte commands" $true
            ProcessToolCommandsWithPSSession -Toolname "latte" -RecvPSSession $recvPSSession -SendPSSession $sendPSSession -CommandsDir $workingDir -Bcleanup $Bcleanup -BZip $ZipResults -TimeoutValueBetweenCommandPairs $TimeoutValueInSeconds -PollTimeInSeconds $PollTimeInSeconds 
        }

        if (Test-Path -Path "$commandsDir\ncps") {
            LogWrite "Processing ncps commands" $true
            ProcessToolCommandsWithPSSession -Toolname "ncps" -RecvPSSession $recvPSSession -SendPSSession $sendPSSession -CommandsDir $workingDir -Bcleanup $Bcleanup -BZip $ZipResults -TimeoutValueBetweenCommandPairs $TimeoutValueInSeconds -PollTimeInSeconds $PollTimeInSeconds
        }
...
<#
.SYNOPSIS
    This function reads an input file of commands and orchestrates the execution of these commands on remote machines for all tools.

.PARAMETER RecvPSSession
    The PSSession of the destination machine that's going to play the Receiver role and wait to receive data for the duration of the throughput tests

.PARAMETER SendPSSession
    The PSSession of the sender machine that's going to send data for the duration of the throughput tests
...
.PARAMETER PollTimeInSeconds
    Optional parameter to configure the amount of time the tool waits (in seconds) before waking up to check if the TimeoutValueBetweenCommandPairs period has elapsed
    #>
Function ProcessToolCommandsWithPSSession{
    param(
        ...
        [Parameter(Mandatory=$True)] [string]$CommandsDir,
        [Parameter(Mandatory=$True)] [string]$Bcleanup, 
        [Parameter(Mandatory=$False)] [string]$Tool = "ntttcp", 
        [Parameter(Mandatory=$True)] [bool]$BZip,
        [Parameter(Mandatory=$False)] [int] $TimeoutValueBetweenCommandPairs = 60,
        [Parameter(Mandatory=$False)] [int] $PollTimeInSeconds = 5
        )
        ...
        try {
            if($recvPSsession -eq $null) {
                LogWrite "Recv PS Session is null"
                return
            }
    
            if($sendPSsession -eq $null) {
                LogWrite "Send PS Session is null"
                return
            }
    ...
    } # ProcessToolCommandsWithPSSession()
